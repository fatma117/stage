...
export class ReportGenerator {
  printConsoleReport(results: PlagiarismResult[]): void {
    ...
    for (let i = 0; i < results.length; i++) {
      ...
      console.log(`${i + 1}. 📄 ${result.file1} ↔ ${result.file2}`);
      ...
      if (result.fragments.length > 0) {
        ...
        result.fragments.slice(0, 3).forEach((fragment, idx) => {
          console.log(`     ${idx + 1}. Lines ${fragment.file1Region.startRow + 1}-${fragment.file1Region.endRow + 1} ↔ Lines ${fragment.file2Region.startRow + 1}-${fragment.file2Region.endRow + 1} [${fragment.length} tokens]`);
        });
  ...
  private createHTMLReport(results: PlagiarismResult[]): string {
    ...
    const maxSimilarity = results.length > 0 ? 
      Math.max(...results.map(r => r.similarity)) : 0;
    const totalFragments = results.reduce((sum, r) => sum + r.fragments.length, 0);
...
<head>
    ...
    <style>
        ...
        * {
        ...
        .similarity-fill.high { background: linear-gradient(90deg, var(--danger-color), #c0392b); }
        .similarity-fill.medium { background: linear-gradient(90deg, var(--warning-color), #d68910); }
        .similarity-fill.low { background: linear-gradient(90deg, var(--success-color), #229954); }
        ...
        .similarity-percent.high { color: var(--danger-color); }
        .similarity-percent.medium { color: var(--warning-color); }
        .similarity-percent.low { color: var(--success-color); }
        ...
        @media (max-width: 768px) {
            ...
            .regions-comparison { grid-template-columns: 1fr; }
            .file-pair { flex-direction: column; align-items: stretch; }
            .stats-row { flex-direction: column; gap: 15px; }
            .result-header { padding: 20px; }
            .result-content { padding: 0 20px 20px 20px; }
        }
    </style>
</head>
<body>
    <div class="container">
        ...
        <div class="summary-cards">
            <div class="summary-card">
                <h3>Suspicious Pairs Found</h3>
                <div class="value">${results.length}</div>
                <div class="subtitle">File pairs above threshold</div>
            </div>
            <div class="summary-card">
                <h3>Maximum Similarity</h3>
                <div class="value">${(maxSimilarity * 100).toFixed(1)}%</div>
                <div class="subtitle">Highest detected similarity</div>
            </div>
            <div class="summary-card">
                <h3>Average Similarity</h3>
                <div class="value">${(avgSimilarity * 100).toFixed(1)}%</div>
                <div class="subtitle">Mean across all pairs</div>
            </div>
            <div class="summary-card">
                <h3>Total Fragments</h3>
                <div class="value">${totalFragments}</div>
                <div class="subtitle">Similar code segments</div>
            </div>
        </div>
        ...
        <div class="results-section">
            ...
            ${results.length === 0 ? `
                <div class="no-results">
                    ...
                    <p>All file pairs are below the similarity threshold.</p>
                </div>
            ` : results.map((result, index) => {
                const similarityPercent = result.similarity * 100;
                const similarityClass = similarityPercent >= 70 ? 'high' : similarityPercent >= 40 ? 'medium' : 'low';
                ...
                <div class="result-item" id="result-${index}">
                    <div class="result-header" onclick="toggleResult(${index})">
                        <div class="result-info">
                            <div class="file-pair">
                                <div class="file-name">${result.file1}</div>
                                <div class="arrow">↔</div>
                                <div class="file-name">${result.file2}</div>
                            </div>
                            ...
                            <div class="similarity-display">
                                ...
                                <div class="similarity-percent ${similarityClass}">${similarityPercent.toFixed(1)}%</div>
                            </div>
                        ...
                        <button class="expand-btn" onclick="event.stopPropagation(); toggleResult(${index})">
                            <span id="btn-text-${index}">View Details</span>
                        </button>
                    </div>
                        ...
                        <div class="fragments-grid">
                            ${result.fragments.length === 0 ? `
                                <p style="text-align: center; color: var(--text-light); padding: 20px;">
                                    No significant fragments detected for this similarity level.
                                </p>
                            ` : result.fragments.map((fragment, fragIndex) => `
                                <div class="fragment-card">
                                    <div class="fragment-header">
                                        <div class="fragment-title">Fragment #${fragIndex + 1}</div>
                                        <div class="fragment-length">${fragment.length} tokens</div>
                                    </div>
                                    
                                    <div class="regions-comparison">
                                        <div class="region-info">
                                            <div class="region-title">📄 ${result.file1}</div>
                                            <div class="region-details">
                                                Lines ${fragment.file1Region.startRow + 1}:${fragment.file1Region.startCol + 1} - ${fragment.file1Region.endRow + 1}:${fragment.file1Region.endCol + 1}
                                            </div>
                                        </div>
                                        <div class="region-info">
                                            <div class="region-title">📄 ${result.file2}</div>
                                            <div class="region-details">
                                                Lines ${fragment.file2Region.startRow + 1}:${fragment.file2Region.startCol + 1} - ${fragment.file2Region.endRow + 1}:${fragment.file2Region.endCol + 1}
                                            </div>
                                        </div>
                                    </div>
                                    ...
                                    ${fragment.matchedTokens.length > 0 ? `
                                        <div class="token-preview">
                                            <strong>📝 Matched tokens:</strong><br>
                                            ${fragment.matchedTokens.slice(0, 30).join(' ')}${fragment.matchedTokens.length > 30 ? '...' : ''}
                                        </div>
                                    ` : ''}
                                </div>
                            `).join('')}
        ...
        <div class="footer">
            <p>🔍 Generated by Pascal Plagiarism Detector | Based on Dolos winnowing algorithms</p>
        </div>
    </div>
